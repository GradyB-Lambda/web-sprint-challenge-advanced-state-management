...
1. What problem does the context API help solve?
   Excessive prop drilling when sharing state down a component tree.
2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
   Actions: are events that describe something that happened in the application; e.g. adding a new todo, the toggle of an item, clearing items from a list.
   Reducers: are function that state the action, and a the current state, and return a new state.
   Store: holds the application state at a global level for components to utilize.
3. What is the difference between Application state and Component state? When would be a good time to use one over the other?
   Application State: reffers to state that is utilized through multiple componenets at different levels of the application.
   Componenet State: applies to state found in a component and is not accessible from other componenets untless passed through props. This is best used when state is not generally needed in other parts of the application.
4. Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
   Allows us to handle asynchonous actions, by returning function than than actions.Actions creators ususally return an object, but with thunk they can return functions.
